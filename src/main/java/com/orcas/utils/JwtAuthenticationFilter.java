...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
	...
	private final List<String> skipServices = Arrays.asList(MappingConstants.URL_API_V1 + MappingConstants.URL_LOGIN,
			MappingConstants.URL_API_V1 + MappingConstants.HEALTH_CHECK,
			"/privacy.html");
		...
		if (skipServices.contains(request.getRequestURI()) || "OPTIONS".equals(request.getMethod())) {
			...
			if (authorizationHeader != null && authorizationHeader.startsWith("Bearer ")) {
				...
				if (jwtUtil.isExpired(jwt)) {
					...
					if (Arrays.asList("POST", "PATCH", "DELETE").contains(request.getMethod())
							&& !jwtUtil.extractRole(jwt).equals("A")) {
